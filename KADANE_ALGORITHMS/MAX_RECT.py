			...
			for r2 in range(R):
				for c2 in range(C):
					if( r <= r2 and c <= c2 and  ( (r2 + 1 - r)!= R or (c2 + 1 - c) !=C) ):
						#sum += mat[r:r2][c:c2] 
						if( (  (r2 + 1 - r) * (c2 + 1 - c)  )/ ( r2 + 1 - r)   !=     ( r2 + 1 - r ) and (  (r2 + 1 - r) * (c2 + 1 - c)  )/ (c2 + 1 - c)   !=     (c2 + 1 - c)  and (c2 + 1 - c) != ( r2 + 1 - r)  ):
							#if( (  (r2 + 1 - r) * (c2 + 1 - c)  )/ ( r2 + 1 - r)   ==     (c2 + 1 - c)  and (  (r2 + 1 - r) * (c2 + 1 - c)  )/ (c2 + 1 - c)   ==     (r2 + 1 - r)):
						
								# always a rectangle
							aux = [[0 for ii in range(C)] for jj in range(R)]  # N col, M row
							
							preProcess(mat, aux)
							sum = sumQuery(aux, r, c, r2, c2)
							if (sum > max_sum):
								max_sum = sum;
...
def sumQuery(aux, top, left, bottom, right):
