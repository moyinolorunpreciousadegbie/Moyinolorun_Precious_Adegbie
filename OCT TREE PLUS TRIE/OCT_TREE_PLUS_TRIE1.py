...
def lll(listt, Level ):
	...
	while q: #                              [     [ ....[ [X1,X2,X_CUT]  ,   [Y1,Y2,Y_CUT]  ,   [Z1,Z2,Z_CUT]  ] ,,,,, ]         ]
		...
		if len(qq) == Level :
			pass
			count += 1
			#print(qq,"qq",Level, count, TTRRIIEE1 , TTRRIIEE2 )
			#print(qq,"qq",Level, count )
				...
				for y in pool :
					...
					if  len(  XX_YY_ZZ  ) == ( len(iterables)  ) : 
						...
						if qqq != tuple(XX_YY_ZZ)  :
							...
							#q.append( (qq+[tuple(XX_YY_ZZ)] , TTRRIIEE2 , TTRRIIEE2_NEW  ) )
							#  TTRRIIEE1.children[qqq] = TTRRIIEE2
							q.append( (qq+[tuple(XX_YY_ZZ)] , TTRRIIEE1.children[qqq] , TTRRIIEE2_NEW  ) )
