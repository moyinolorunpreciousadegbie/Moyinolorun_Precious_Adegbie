#Leetcode1998. GCD Sort of an Array   BFS
...
def do(A):
	...
	AA = [A]
	q = []
	for ii in range( 2, len(AA[0]) +1):
		for t in range(len(AA[0])-1):
			for h in range(0,ii,ii-1):
				if(h+t < len(AA[0]) and ii- h + t - 1 < len(AA[0]) ):
