...
def add(wells, tar, tar2):
	...
	for i in range(2, len(wells)+1):
		...
		for wellz in list(combinations(wells, i))  :
			...
			#print(edges)
			findShortestCycle(n, edges, tar, tar2) 
			print()
			print()
			print()
			print()
...
def findShortestCycle(n, edges, tar, tar2) :
		...
		for u , vv ,tocc, gap in edges: 
			...
			while queue1: 
				...
				if tar <= sum(path) <= tar2 : 
					ln = len(path)
					print(path[1:],'     path in range          ',tar,'<=',sum(path),'<=',tar2)
					
					print(tooc[:ln-1],'     TOC so far            total TOC value',sum(tooc[:ln-1]))
